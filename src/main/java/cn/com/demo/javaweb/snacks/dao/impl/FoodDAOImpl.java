...
/**
 * @author hj
 * @time 2019年7月26日上午2:34:31 state：
 */
public class FoodDAOImpl implements IFoodDAO {
	...
	public int findFoodSaleCount(int fdId) {
		...
		try {
			conn = dbConn.getConnection();
			String sql = "select sum(it.od_id_count) from snacks_order_item it,snacks_order";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, fdId);
			pstmt.setInt(2, Constant.ORDER_STATUS_OK);// 9表示完成订单状态
		} catch (Exception e) {
